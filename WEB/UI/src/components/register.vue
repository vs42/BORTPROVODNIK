                ...
                if (!(this.input.username != "" && this.input.password != "" &&
                    ...
                    if(!this.empty) {
                        if (this.input.password != this.input.confirm_password) {
                            ...
                            axios.post('http://172.31.226.63:80/api/register', {username: this.input.username, password: this.input.password, name: this.input.email}, { headers: { 'Content-Type': 'application/json' }}).then(function (response) {
                                localStorage.clear();
